...
<body class="bg-stone-50 text-stone-800 font-sans antialiased">
  ...
  <nav id="navbar" class="fixed top-0 inset-x-0 z-50 transition-all duration-300 py-5 px-6 lg:px-12 flex items-center justify-between">
    <a href="#home" class="flex items-center gap-3 font-serif text-xl font-semibold text-white tracking-wide drop-shadow" style="font-family:'Nunito',sans-serif;">
      <img src="<?= h($logoUrl) ?>" alt="DROMUS logo" class="w-10 h-10 rounded-full object-cover border border-white/40 shadow-md" />
      <span><?= h($text($texts, 'brand.name', 'Dromus Bed & Boetiek')) ?></span>
    </a>
    <ul class="hidden md:flex gap-8 text-sm font-medium text-white/90 drop-shadow">
      <li><a href="#room" class="hover:text-white transition-colors">De Kamer</a></li>
    ...
    <button id="menuBtn" class="md:hidden text-white focus:outline-none" aria-label="Menu openen">
  ...
  <div id="mobileMenu" class="hidden fixed inset-0 z-40 bg-stone-900/95 flex flex-col items-center justify-center gap-10 text-2xl font-medium text-white">
    <a href="#room" class="hover:text-sand transition-colors" onclick="closeMobileMenu()">De Kamer</a>
    <a href="#reviews" class="hover:text-sand transition-colors" onclick="closeMobileMenu()">Reviews</a>
  ...
  <section id="home" class="relative w-full h-screen overflow-hidden">
    ...
    <div class="relative z-10 flex flex-col items-center justify-center h-full text-center px-6">
      <img src="<?= h($logoUrl) ?>" alt="DROMUS Bed &amp; Boetiek logo" class="w-[7.5rem] h-[7.5rem] md:w-[9rem] md:h-[9rem] rounded-full object-cover border-[3px] border-white/75 shadow-2xl mb-5" />
      <p class="text-sand-light uppercase tracking-widest text-sm mb-3 font-medium"><?= h($text($texts, 'hero.eyebrow', 'Welkom bij')) ?></p>
      ...
      <p class="text-white/80 text-lg md:text-xl max-w-xl mb-10 font-light"><?= h($text($texts, 'hero.description', 'Geniet van een unieke verblijfservaring in ons stijlvol ingerichte gastenkamer, midden in het hart van de stad.')) ?></p>
  ...
  <section id="room" class="py-24 px-6 lg:px-20 bg-sand-light">
    <div class="max-w-6xl mx-auto">
      <div class="text-center mb-16">
        ...
        <h2 class="font-serif text-4xl md:text-5xl text-stone-800 mb-5"><?= h($text($texts, 'room.title', 'De Gastenkamer')) ?></h2>
      ...
      <div class="grid md:grid-cols-2 gap-12 items-center mb-16">
        <div>
          ...
          <p class="text-stone-600 leading-relaxed mb-5"><?= h($text($texts, 'room.description_1', 'Onze ruime, lichtrijke gastenkamer combineert hedendaags comfort met een warme, gezellige sfeer.')) ?></p>
        ...
        <div>
          <?php if ($roomMain): ?>
            <img src="<?= h($assetUrl((string)$roomMain->image_url)) ?>" alt="<?= h((string)($roomMain->alt_text ?: 'Gastenkamer overzicht')) ?>" class="gallery-img w-full h-80 md:h-96 object-cover rounded-2xl shadow-xl" />
          <?php endif; ?>
        </div>
      </div>
  ...
  <section id="location" class="py-24 px-6 lg:px-20 bg-stone-50">
    <div class="max-w-6xl mx-auto">
  ...
  <script>
    ...
    window.addEventListener('scroll', () => {
      if (window.scrollY > 60) {
        navbar.classList.add('bg-stone-900/95', 'shadow-md', 'py-3', 'backdrop-blur-sm');
        navbar.classList.remove('py-5');
      } else {
        navbar.classList.remove('bg-stone-900/95', 'shadow-md', 'py-3', 'backdrop-blur-sm');
